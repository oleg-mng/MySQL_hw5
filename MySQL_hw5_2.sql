...
SELECT firstname, lastname, pr.*
FROM users, (SELECT photo_id, hometown, user_id FROM profiles) AS pr
WHERE users.id = pr.user_id;

SELECT id, firstname, lastname, photo_id, hometown
FROM users, profiles
...
SELECT media.filename, media_types.name_type, media.media_type_id
FROM media
JOIN media_types
...
SELECT status, initiator_user_id, users.firstname, target_user_id
FROM friend_requests
JOIN users
ON users.id = friend_requests.initiator_user_id
WHERE friend_requests.status = 'approved' AND users.id = 1;
